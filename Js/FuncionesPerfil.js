document.addEventListener('DOMContentLoaded', () => {
    const fotoContainer = document.querySelector('.w-24.h-24.cursor-pointer'); // contenedor de la imagen
    const btnSeleccionar = document.querySelector('button[type="button"].text-xs.bg-rose-800\\/20'); // boton de seleccionar
    ...
    if (inputFoto) {
        // Al hacer clic en el círculo de la foto
        if (fotoContainer) {
            fotoContainer.addEventListener('click', () => inputFoto.click()); //si hacen click se abre el explorador de archivos
        }

        // Al hacer clic en el botón de seleccionar
        if (btnSeleccionar) {
            btnSeleccionar.addEventListener('click', () => inputFoto.click()); // si hacen click se abre el explorador de archivos
        }

        // Al cambiar el archivo seleccionado (Vista previa)
        inputFoto.addEventListener('change', (event) => { // se activa justo después de seleccionar un archivo del ordenador
            if (event.target.files && event.target.files[0] && imageField) { // Comprobación de que hay seleccionado un archivo
                const reader = new FileReader(); // Este objeto es un lector, que permite ver archivos del pc y ser entendidos por el navegador
                reader.onload = (e) => { // cuando se termina de leer el archivo se pone como nueva SRC de la imagen.
    ...
    if (bioTextarea) {
        ...
        counter.className = "text-right text-xs font-bold mt-1 transition-colors text-zinc-500"; // Le doy estilos //
        ...
        const updateCounter = () => {
            ...
            if (count > 1000) {  // filtro de los carácteres //
