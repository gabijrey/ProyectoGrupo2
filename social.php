...
$sql_podio = "SELECT u.nombre, u.img_perfil, COUNT(r.id) as total_resenas 
              FROM usuario u 
...
$sql_recientes = "SELECT r.*, o.titulo, o.portada, u.img_perfil 
                  FROM resena r 
...
$sql_nuevos_users = "SELECT nombre, img_perfil, fecha_registro 
                     FROM usuario 
        ...
        <div class="w-full max-w-[1850px] grid grid-cols-1 xl:grid-cols-[380px_1fr_380px] gap-16 items-start">
            ...
            <aside class="hidden xl:flex flex-col gap-8">
                ...
                <div class="space-y-8">
                    ...
                    <a href="webcontent/obra.php?id=<?= $r['id_obra'] ?>" class="block bg-white border-4 border-black p-4 shadow-[8px_8px_0_0_black] hover:translate-x-2 hover:-translate-y-2 transition-transform cursor-pointer group">
                        <div class="flex items-center gap-3 mb-3">
                            <img src="<?= $r['img_perfil'] ?>" class="w-12 h-12 rounded-full border-2 border-black object-cover">
                            <span class="font-comic text-black text-lg uppercase truncate group-hover:text-rose-800 transition-colors"><?= $r['nombre_usuario'] ?></span>
                        </div>
            ...
            <div class="flex flex-col items-center w-full">
                ...
                <div class="flex items-end justify-center gap-4 md:gap-8 w-full max-w-4xl mt-10">
                    ...
                    <div class="flex flex-col items-center w-32 md:w-48 transition-transform hover:scale-105">
                        <a href="perfilUsuario.php?usuario=<?= urlencode($user['nombre']) ?>" class="relative mb-4 cursor-pointer group">
                        ...
                        <div class="<?= $height_class ?> <?= $color_class ?> w-full border-x-4 border-t-4 border-black shadow-[8px_0_0_0_black] flex flex-col items-center justify-start pt-4 px-2 text-center">
                            <a href="perfilUsuario.php?usuario=<?= urlencode($user['nombre']) ?>" class="font-comic text-xl md:text-2xl text-black truncate w-full uppercase hover:text-rose-800 transition-colors cursor-pointer"><?= $user['nombre'] ?></a>
            ...
            <aside class="hidden xl:flex flex-col gap-8">
                ...
                <div class="grid grid-cols-1 gap-6">
                    ...
                    <a href="perfilUsuario.php?usuario=<?= urlencode($un['nombre']) ?>" class="bg-neutral-800 border-4 border-black p-6 flex items-center gap-6 hover:bg-yellow-500 hover:text-black transition-all group shadow-[8px_8px_0_0_black] hover:-translate-y-2">
                        ...
                        <div class="min-w-0">
                            <h4 class="font-comic text-2xl uppercase truncate text-white group-hover:text-black leading-none mb-2"><?= $un['nombre'] ?></h4>
