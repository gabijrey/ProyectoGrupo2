...
$stmtComicsTop = $_conexion->prepare("
    ...
    LIMIT 5
");
$stmtComicsTop->execute();
$comicsTop = $stmtComicsTop->fetchAll(PDO::FETCH_ASSOC);

$stmtCategorias = $_conexion->prepare("
    SELECT DISTINCT genero 
    FROM obra 
    WHERE tipo IN (0, 3) AND genero IS NOT NULL AND genero != ''
"); // Soy gilipollas, y puse 3 en vez de 0 para los cómics, así que tengo que incluir el 3 también para que salgan las categorías.
...
<body class="text-white flex flex-col min-h-screen">
    ...
    <nav class="bg-neutral-900 p-6 grid grid-cols-1 md:grid-cols-3 items-center border-b-8 border-black sticky top-0 z-50">
        ...
        <div class="flex justify-center items-center gap-8 font-comic text-xl">
            ...
            <a href="comics.php" class="text-rose-800 hover:text-white transition-colors uppercase tracking-widest">Comics</a>
            <a href="mangas.php" class="hover:text-rose-800 transition-colors uppercase tracking-widest">Mangas</a>
        ...
        <section class="fade-up delay-4 mt-8 w-full max-w-7xl">
            ...
            <div class="mb-16">
                ...
                <div class="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-5 gap-8">
                    <?php foreach ($comicsTop as $comic){ ?>
                        <div class="flex flex-col items-center group">
                            <div class="w-full aspect-[2/3] bg-white border-4 border-black overflow-hidden shadow-[8px_8px_0_0_black] transition-all group-hover:translate-x-1 group-hover:translate-y-1 group-hover:shadow-none">
                                <a href="obra.php?id=<?= $comic['id'] ?>">
                                    <img class="w-full h-full object-cover grayscale hover:grayscale-0 transition-all duration-500" src="imagen/<?= htmlspecialchars($comic['portada']) ?>" alt="<?= htmlspecialchars($comic['titulo']) ?>">
                                </a>
                            </div>
                            <div class="mt-4 bg-white border-4 border-black p-3 w-full shadow-[4px_4px_0_0_black]">
                                <p class="text-black text-sm font-bold uppercase truncate text-center mb-1" title="<?= htmlspecialchars($comic['titulo']) ?>"><?= htmlspecialchars($comic['titulo']) ?></p>
                                <p class="text-center font-comic text-rose-800 text-xl leading-none">
                                    <?= number_format($comic['media'], 1) ?> <span class="text-yellow-500 drop-shadow-[1px_1px_0_black]">★</span>
                                </p>
                                <a class="block w-full text-center bg-rose-800 text-white font-bold py-1 border-2 border-black mt-2 hover:bg-rose-900 transition-all text-xs" href="obra.php?id=<?= $comic['id'] ?>">
                                    VER MÁS
                                </a>
                            </div>
                        </div>
            ...
            <?php foreach ($categorias as $categoria){ 
                $stmtComics = $_conexion->prepare("
                    SELECT * FROM obra 
                    WHERE tipo IN (0, 3) AND genero = :genero 
                    ORDER BY id DESC 
                    LIMIT 5
                ");
                $stmtComics->execute(['genero' => $categoria]);
                $comicsCategorias = $stmtComics->fetchAll(PDO::FETCH_ASSOC);
                
                if (count($comicsCategorias) > 0){
            ?>
            <div class="mb-16">
                ...
                <div class="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-5 gap-8">
                    <?php foreach ($comicsCategorias as $comic){ ?>
                        <div class="flex flex-col items-center group">
                            <div class="w-full aspect-[2/3] bg-white border-4 border-black overflow-hidden shadow-[8px_8px_0_0_black] transition-all group-hover:translate-x-1 group-hover:translate-y-1 group-hover:shadow-none">
                                <a href="obra.php?id=<?= $comic['id'] ?>">
                                    <img class="w-full h-full object-cover grayscale hover:grayscale-0 transition-all duration-500" src="imagen/<?= htmlspecialchars($comic['portada']) ?>" alt="<?= htmlspecialchars($comic['titulo']) ?>">
