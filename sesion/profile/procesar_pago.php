...
try {
    ...
    $ins = $_conexion->prepare("
        INSERT INTO suscripcion (id_usuario, metodo_pago, estado, fecha_inicio, fecha_cancelacion)
        VALUES (:id, :metodo, 1, CURDATE(), NULL)
    ");
    $ins->execute(['id' => $id_usuario, 'metodo' => $metodo_pago]);

    // Actualizar el rol del usuario
    $upd2 = $_conexion->prepare("UPDATE usuario SET rol = :rol WHERE id = :id");
    $upd2->execute(['rol' => $rol_nuevo, 'id' => $id_usuario]);
